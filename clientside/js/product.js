...
async function getProduct() {
    const res=await fetch(`http://localhost:3001/api/getproduct/${id}`);
    product=await res.json();
    const res2=await fetch(`http://localhost:3001/api/getuser/${product.sellerId}`);
...
async function book() {
        fetch("http://localhost:3001/api/setBooking",{
            method:"POST",
